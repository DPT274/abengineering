...
// Dữ liệu gốc đang lưu ở dạng tĩnh
let globalAboutData = {
...
router.post("/api/settings/about", (req, res) => {
    try {
        const d = req.body;
        // Chúng ta cập nhật lại toàn bộ object dữ liệu
        globalAboutData = {
            companyName: d.companyName || "", slogan: d.slogan || "", bannerImg: d.bannerImg || "",
            aboutText: d.aboutText || "", statsImg: d.statsImg || "", visionText: d.visionText || "", missionText: d.missionText || "",
            // Cập nhật từng biến đơn lẻ để giữ tương thích 100% với file User của bạn
